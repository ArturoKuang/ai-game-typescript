...
- There is no top-level `package.json` in `tools/`; run commands from `extractor/` or `visualizer/`.
- If the visualizer looks stale, re-run `cd tools/extractor && npm run extract` so `public/graph.json` is refreshed.
