/**
 * Architecture graph schema shared by the extractor and visualizer.
 *
 * Audit note: edit this file when the `graph.json` contract changes. The
 * package-local `types.ts` files in each tool are only re-export shims.
 */
...
/** Visualizer-only zoom modes still live in the shared schema module. */
