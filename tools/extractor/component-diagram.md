...
flowchart TB
  ...
  subgraph component_view_browser_client_system["AI Town"]
    ...
    subgraph component_view_browser_client_boundary["Browser Client"]
      ...
      component_view_browser_client_conversation_ui["Conversation UI<br/>DOM APIs<br/>Owns: chat • player list • conversation panel • status bar<br/>Ingress: convo_update • message • player_update<br/>Egress: start_convo • accept_convo • decline_convo • say • end_convo<br/>Depends On: App Shell callbacks • DOM controls<br/>Internals: updatePlayerList() • renderConversationPanel() • addChatMessage()"]
    end
  end
