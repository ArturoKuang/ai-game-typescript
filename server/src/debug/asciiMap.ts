 ...
 */
export function renderAsciiMap(game: GameLoop): { ascii: string; legend: Record<string, string> } {
  ...
  for (const activity of world.getActivities()) {
    if (activity.y >= 0 && activity.y < world.height && activity.x >= 0 && activity.x < world.width) {
  ...
  for (const p of players) {
    ...
    const stateStr = p.state === 'idle' ? 'idle' : p.state === 'walking' ? `walking→(${p.targetX},${p.targetY})` : p.state;
  ...
  for (const p of players) {
    if (p.currentConvoId && !linked.has(p.id)) {
      const partner = players.find(
        other => other.id !== p.id && other.currentConvoId === p.currentConvoId,
      );
      if (partner) {
        linked.add(p.id);
