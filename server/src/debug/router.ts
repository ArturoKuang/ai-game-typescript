...
async function persistPlayer(pool: Pool | undefined, player: Player): Promise<void> {
  if (!pool) return;
  await pool.query(
    `INSERT INTO players (id, name, description, personality, is_npc, x, y, state)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
     ON CONFLICT (id) DO UPDATE SET x = $6, y = $7, state = $8`,
    [player.id, player.name, player.description, player.personality ?? null, player.isNpc, player.x, player.y, player.state],
  );
}

export function createDebugRouter(game: GameLoop, memoryManager?: MemoryManager, pool?: Pool): Router {
  const router = Router();
  ...
  if (memoryManager) {
    router.get('/memories/:playerId', async (req, res) => {
      try {
        ...
        const memories = await memoryManager.getMemories(req.params.playerId, { limit, type });
        res.json(memories);
      } catch (err: any) {
    ...
    router.get('/memories/:playerId/search', async (req, res) => {
      try {
        ...
        const k = req.query.k ? parseInt(req.query.k as string, 10) : 5;
    ...
    router.post('/memories', async (req, res) => {
      try {
        ...
        if (!playerId || !type || !content) {
          res.status(400).json({ error: 'Missing required fields: playerId, type, content' });
          return;
        }
    ...
    router.post('/remember-convo', async (req, res) => {
      try {
        ...
        for (const pid of [convo.player1Id, convo.player2Id]) {
          const partner = pid === convo.player1Id ? convo.player2Id : convo.player1Id;
