...
import type { ClientMessage, ServerMessage, FullGameState } from './protocol.js';
...
export class GameWebSocketServer {
  ...
  constructor(server: Server, game: GameLoop, convoManager: ConversationManager) {
    this.game = game;
    ...
    switch (msg.type) {
      ...
      case 'say': {
        ...
        if (!convo || convo.state !== 'active') {
          this.send(ws, { type: 'error', data: { message: 'Not in an active conversation' } });
          return;
        }
        const chatMsg = this.convoManager.addMessage(convo.id, info.playerId, msg.data.content, this.game.currentTick);
      ...
      case 'start_convo': {
        if (!info.playerId) return;
        try {
          const convo = this.convoManager.startConversation(info.playerId, msg.data.targetId, this.game.currentTick);
          this.broadcast({ type: 'convo_update', data: convo });
        } catch (err: any) {
