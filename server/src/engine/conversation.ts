...
export class ConversationManager {
  ...
  startConversation(player1Id: string, player2Id: string, tick: number): Conversation {
    // Check neither player is already in a conversation
    for (const c of this.conversations.values()) {
      if (c.state === 'ended') continue;
      if (c.player1Id === player1Id || c.player2Id === player1Id ||
          c.player1Id === player2Id || c.player2Id === player2Id) {
  ...
  acceptInvite(convoId: number): Conversation {
    ...
    if (convo.state !== 'invited') throw new Error('Conversation is not in invited state');
    convo.state = 'walking';
    return convo;
  }

  addMessage(convoId: number, playerId: string, content: string, tick: number): Message {
    const convo = this.getConversation(convoId);
    if (!convo) throw new Error(`Conversation ${convoId} not found`);
  ...
  processTick(
    ...
    for (const convo of this.conversations.values()) {
      if (convo.state === 'invited') {
        ...
        if (p2?.isNpc) {
          convo.state = 'walking';
          events.push({ tick, type: 'convo_accepted', data: { convoId: convo.id } });
        }
      }
      ...
      if (convo.state === 'walking') {
        ...
        if (dist <= CONVERSATION_DISTANCE) {
          ...
          convo.state = 'active';
          events.push({ tick, type: 'convo_active', data: { convoId: convo.id } });
        } else {
      ...
      if (convo.state === 'active') {
        ...
        if (tick - convo.startedTick >= MAX_DURATION) {
          this.endConversation(convo.id, tick);
          events.push({ tick, type: 'convo_ended', data: { convoId: convo.id, reason: 'max_duration' } });
          continue;
        }
        ...
        if (convo.messages.length >= MAX_MESSAGES) {
          this.endConversation(convo.id, tick);
          events.push({ tick, type: 'convo_ended', data: { convoId: convo.id, reason: 'max_messages' } });
          continue;
        }

        // Check timeout (no messages for N ticks)
        const lastMsgTick = convo.messages.length > 0
          ? convo.messages[convo.messages.length - 1].tick
          : convo.startedTick;
        if (tick - lastMsgTick >= CONVERSATION_TIMEOUT) {
          this.endConversation(convo.id, tick);
          events.push({ tick, type: 'convo_ended', data: { convoId: convo.id, reason: 'timeout' } });
          continue;
        }
      }
    }
  ...
  getActiveConversations(): Conversation[] {
    return Array.from(this.conversations.values()).filter(c => c.state !== 'ended');
  }
