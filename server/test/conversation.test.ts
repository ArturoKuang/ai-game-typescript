...
describe('ConversationManager', () => {
  ...
  it('rejects conversation if player is already in one', () => {
    ...
    expect(() => cm.startConversation('alice', 'carol', 0)).toThrow('already in a conversation');
    expect(() => cm.startConversation('carol', 'bob', 0)).toThrow('already in a conversation');
  });
  ...
  it('rejects messages from non-participants', () => {
    ...
    const convo = cm.startConversation('alice', 'bob', 0);
    convo.state = 'active';
    expect(() => cm.addMessage(convo.id, 'carol', 'Hey!', 1)).toThrow('not part of');
  });

  it('rejects messages in non-active conversations', () => {
    cm = new ConversationManager();
    const convo = cm.startConversation('alice', 'bob', 0);
    expect(() => cm.addMessage(convo.id, 'alice', 'Hello', 1)).toThrow('not active');
  });
...
describe('Conversation Integration with GameLoop', () => {
  ...
  it('messages can be exchanged in active conversation', () => {
    ...
    const msg = tg.game.conversations.addMessage(convo.id, 'alice', 'Hello Bob!', tg.game.currentTick);
