...
describe('Reflection (unit)', () => {
  ...
  it('memory importance scoring produces values in range', () => {
    ...
    const score = (msgCount: number) => Math.min(9, Math.max(1, Math.ceil(msgCount * 1.5)));
  ...
  it('recency decay produces expected scores', () => {
    const decay = 0.99;
    // 0 ticks ago -> 1.0
    expect(Math.pow(decay, 0)).toBeCloseTo(1.0);
    // 100 ticks ago -> ~0.366
    expect(Math.pow(decay, 100)).toBeCloseTo(0.366, 2);
    // 500 ticks ago -> ~0.0066
    expect(Math.pow(decay, 500)).toBeCloseTo(0.00657, 3);
  });
});
